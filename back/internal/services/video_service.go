...
type VideoServiceInterface interface {
	CreateVideo(userID int64, title string, file multipart.File, filename string) (string, error)
...
func (s *VideoService) CreateVideo(userID int64, title string, file multipart.File, filename string) (string, error) {
	id := uuid.New().String()
	...
	_, err := s.db.Exec(`INSERT INTO videos (video_id, user_id, title, original_filename, original_url, status, uploaded_at) VALUES ($1,$2,$3,$4,$5,$6,$7)`,
		id, userID, title, filename, origPath, "uploaded", uploadedAt)
	if err != nil {
		return "", err
	}
	return id, nil
...
func (s *VideoService) GetVideosByUser(userID int64) ([]models.Video, error) {
	rows, err := s.db.Query(`SELECT video_id, title, original_filename, original_url, status, uploaded_at, processed_at, processed_url FROM videos WHERE user_id=$1 ORDER BY uploaded_at DESC`, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var v models.Video
		err := rows.Scan(&v.ID, &v.Title, &v.OriginalFilename, &v.OriginalURL, &v.Status, &v.UploadedAt, &v.ProcessedAt, &v.ProcessedURL)
		if err != nil {
			return nil, err
		}
...
func (s *VideoService) GetVideoByID(videoID string, userID int64) (*models.Video, error) {
	row := s.db.QueryRow(`SELECT video_id, user_id, title, original_filename, original_url, status, uploaded_at, processed_at, processed_url FROM videos WHERE video_id=$1`, videoID)
	var v models.Video
	if err := row.Scan(&v.ID, &v.UserID, &v.Title, &v.OriginalFilename, &v.OriginalURL, &v.Status, &v.UploadedAt, &v.ProcessedAt, &v.ProcessedURL); err != nil {
...
func (s *VideoService) MarkProcessing(videoID string) error {
	_, err := s.db.Exec(`UPDATE videos SET status=$1 WHERE video_id=$2`, "processing", videoID)
	return err
}
...
func (s *VideoService) MarkProcessed(videoID, processedPath string) error {
	...
	_, err := s.db.Exec(`UPDATE videos SET status=$1, processed_url=$2, processed_at=$3 WHERE video_id=$4`, "processed", processedPath, processedAt, videoID)
	return err
}

// MarkFailed anota fallos
func (s *VideoService) MarkFailed(videoID, reason string) error {
	_, err := s.db.Exec(`UPDATE videos SET status=$1 WHERE video_id=$2`, "failed", videoID)
...
func (s *VideoService) DeleteVideo(videoID string, userID int64) error {
	row := s.db.QueryRow(`SELECT user_id, status, original_url, processed_url FROM videos WHERE video_id=$1`, videoID)
	var owner int64
	...
	_, err := s.db.Exec(`DELETE FROM videos WHERE video_id=$1`, videoID)
	return err
}
