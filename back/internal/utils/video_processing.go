...
// AddWatermark overlay watermarkImage at bottom-right with 10px padding
func AddWatermark(src, dst, watermarkImage string) error {
	args := []string{"-i", src, "-i", watermarkImage, "-filter_complex", "overlay=main_w-overlay_w-10:main_h-overlay_h-10", "-codec:a", "copy", "-y", dst}
	return runCmd("ffmpeg", args...)
}
